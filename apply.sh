...
if test "$(uname)" = "Darwin"; then
    ...
    defaults write com.googlecode.iterm2 'Custom Color Presets' -dict-add \
        "material design" "$(cat $script_dir/iTerm/material-design-colors.itermcolors)"
    ...
    sudo tic -xe tmux-256color ${script_dir}/iTerm/tmux-256color.info
    ...
    if cmd_exists autojump; then
        ...
        brew install autojump clang-format cloc cmake cpplint htop llvm mosh ninja \
             opencv@3 sshuttle the_silver_searcher tmux tree vim watch wget
...
if test "$(uname)" = "Linux"; then # ubuntu
    ...
    sudo apt install autojump build-essential clang-format cloc cmake \
        golang mosh python3.8-dev silversearcher-ag tmux tree vim zsh -y
